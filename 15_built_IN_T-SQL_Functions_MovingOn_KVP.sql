...
/* return all orders placed between 2022 and 2023 */
select OrderID, OrderDate
from Sales.Orders
-- where year(OrderDate) in( '2022','2023')
-- where year(OrderDate) between '2022' and '2023'
where OrderDate between '1/1/2022' and '12/31/2023'
;
go
...
select
	upper(concat(left([FirstName], 3), left([LastName], 4))) as 'ID', FirstName, LastName, HomePhone
from HumanResources.Employees
;
go

/* return the customer company name and the telephone area code in Canada and USA */
select CompanyName, Phone, substring([Phone], 2, 3) as 'Area Code'
from Sales.Customers
...
select
try_parse('06/23/2023' as datetime2 using 'fr-fr') as 'French Date Style'
;
go
